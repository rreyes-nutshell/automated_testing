...
test('test', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'Item: Tasks' }).click();
  await page.getByRole('link', { name: 'Create Invoice', exact: true }).click();
  await page.getByRole('combobox', { name: 'Supplier (Required)' }).click();
  await page.getByRole('combobox', { name: 'Supplier (Required)' }).fill('nutsh');
  await page.getByRole('combobox', { name: 'Supplier (Required)' }).press('Tab');
  await page.getByRole('textbox', { name: 'Number (Required)' }).click();
  await page.getByRole('textbox', { name: 'Number (Required)' }).fill('NS111');
  await page.locator('[id="_FOpt1\\:_FOr1\\:0\\:_FONSr2\\:0\\:MAnt2\\:1\\:pm1\\:r1\\:0\\:ap1\\:SPpgl10"]').click();
  await page.getByRole('textbox', { name: 'Amount' }).click();
  await page.getByRole('textbox', { name: 'Amount' }).fill('1.50');
